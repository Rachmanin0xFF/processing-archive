...
void setup() {
  size(1900, 256, P2D);
  Gradient g = new Gradient("inferno");
  for(int x = 0; x < width; x++) {
    stroke(g.getLerp(x, 0, width));
    line(x, 0, x, height);
  }
}
...
//Returns the frequency of a given note (starts at n=0=C0
float note_to_freq(int n) {
  return musical_exp((float)(n-49-9))*440.f;
}
...
public class VecN {
  ...
  //Just makin' this function so I can create empty vectors quick.
  public VecN(String name, int count) {
...
public VecN cross_vectors(VecN... vectors) {
  ...
  if(vectors.length == vector_components) {
    ...
    cross_product.add(determinant(newMat));
  } else {
...
//typing speed and readability edit
public VecN cp(VecN v) {
  return copy_vec(v);
}

public VecN sigmoid(VecN v) {
  VecN o = new VecN();
  for(int i = 0; i < v.v_list.size(); i++) {
    o.v_list.add(sigmoid(v.v_list.get(i)));
  }
  return o;
}
...
public VecN mult(Matrix m1, VecN v1) {
  VecN newVec = new VecN();
  Matrix transp = transpose(m1);
  for(int i = 0; i < v1.v_list.size(); i++) {
    print("\n\nA: ");
    transp.m_list.get(i).print_info();
    print("B: ");
    v1.print_info();
    newVec.v_list.add(dot(v1, transp.m_list.get(i)));
  }
  return newVec;
}
