...
void setup() {
  ...
  if(filenames!=null)
    for(String s : filenames) {
      println("Converting " + s + " to a .obj file...");
      boolean[][][] b = v.parseFile(loadStrings(s));
      String outLocation = "dataOut/" + s.substring(0, s.length()-4) + ".obj";
      v.exportDataToOBJ(b, outLocation);
    }
}
