...
void draw() {
  background(255);
  if(selectedFile == -1)
    genPlot("input_splat/c_0.csv");
  else if(selectedFile == -2)
    genPlot("EENPOOT/c_random_averaged.csv");
  else if(selectedFile == -3) {
    pushMatrix();
    genPlot("EENPOOT/c_real.csv");
    popMatrix();
    genPlot("EENPOOT/c_random_averaged.csv");
  } else
    genPlot("input_splat/c_" + selectedFile + ".csv");
}
void mouseDragged() {
...
void mouseWheel(MouseEvent event) {
  ...
  selectedFile -= e;
  selectedFile = min(99, max(selectedFile, -3));
}
void keyPressed() {
...
PVector majorTick = new PVector(10.f, 5);
PVector minorTick = new PVector(10.f, 1);
float tickSizeX = 0.f;
float tickSizeY = 0.f;
String xAxis = "Hyperedge Degree";
String yAxis = "Impact Score";
void genPlot(String file) {
  ...
  strokeWeight(6);
  if(file.equals("EENPOOT/c_real.csv"))
    stroke(0, 0, 255);
  else if(file.equals("EENPOOT/c_random_averaged.csv"))
    stroke(255, 0, 0);
  for(PVector p : data) {
    mapPoint(p.x, p.y);
  }
...
public PVector[] loadData(String location) {
  String[] f = loadStrings("splat_input/c_0.csv");
  int offset = 0;
