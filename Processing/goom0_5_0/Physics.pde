void physics(Phys ob) {
  ...
  stroke(0);
  if (debug)println("####################### BEGIN FIZZ #######################");
  if (debug)println("V:", av.x, av.y);
  ob.contacting=false;
  for (int run=0; run<2; run++) {
    ...
    for (int ei=0; ei<local.size(); ei++) {
      ...
      if (eIntersect||fIntersect||gIntersect||inside)if (debug)println(eIntersect, fIntersect, gIntersect, inside, "\t", j.intersectOpenInterval(e), k.intersectOpenInterval(e), l.intersectOpenInterval(e), m.intersectOpenInterval(e));
      if (eIntersect||fIntersect||gIntersect||inside) {
        if (debug)println("edge: ", ei, "\t", f.a.x, f.a.y, f.b.x, f.b.y);
    ...
    if (debug)println(dist, ob.x, ob.y, "V:", av.x, av.y);
  }
}
...
void drawline(int chunk, int u, int v, float x1, float y1, float x2, float y2) {
  x1*=GRID;
  y1*=GRID;
  x2*=GRID;
  y2*=GRID;
  line((chunk*Chunk.U+u)*GRID+x1, v*GRID+y1, (chunk*Chunk.U+u)*GRID+x2, v*GRID+y2);
  //line(x,y,x+dx,y+dy);
}
void renderSpecular(int chunk, int u, int v, int radius) {
  strokeWeight(1);
  for (int i=u-radius; i<u+radius; i++) {
    for (int j=max(0, v-radius); j<min(Chunk.V-1, v+radius); j++) {
      float dist=dist(u, v, i, j);
      stroke(255, (radius-dist)*20);
      int shape=getBlockPhys(chunk, i, j);
      int block=getBlock(chunk, i, j);
      if (block%1000==1||block%1000==2||block%1000==3||block%1000==4||block%1000==5) {
        boolean above=getBlock(chunk, i, j-1)==0;
        ...
        if (above) {
          if (subID==1) {
            //Left Ramp
            drawline(chunk, i, j, 0, 1, 1, 0);
            //drawline(chunk, i, j, 1, 0, 1, 1);
          } else if (subID==2) {
            //Left Ramp
            drawline(chunk, i, j, 0, 0, 1, 1);
            //drawline(chunk, i, j, 0, 0, 0, 1);
          } else if (subID==4) {

            drawline(chunk, i, j, 0.2, 0.3, 0.8, 0.3);//top
            drawline(chunk, i, j, 0.0, 0.5, 0.0, 1.0);//Lside
            drawline(chunk, i, j, 1.0, 0.5, 1.0, 1.0);//Rside
            drawline(chunk, i, j, 0.2, 0.3, 0.0, 0.5);//Lramp
            drawline(chunk, i, j, 0.8, 0.3, 1.0, 0.5);//Rramp
          } else if (subID==5) {
            //Left Ramp
            drawline(chunk, i, j, 0.2, 0.8, 1.0, 0.0);
            drawline(chunk, i, j, 0.2, 0.8, 1.0, 1.0);
            //drawline(chunk, i, j, 1, 0, 1, 1);
          } else if (subID==6) {
            //Left Ramp
            drawline(chunk, i, j, 0.0, 0.0, 0.8, 0.8);
            drawline(chunk, i, j, 0.0, 1.0, 0.8, 0.8);
            //drawline(chunk, i, j, 1, 0, 1, 1);
          } else {
            //Regular
            drawline(chunk, i, j, 0, 0, 1, 0);
            //drawline(chunk, i, j, 0, 0, 0, 1);
            //drawline(chunk, i, j, 1, 0, 1, 1);
          }
        }
