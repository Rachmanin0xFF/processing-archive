//THIS VERSION MODIFIED FOR USE WITH THIS SPECIFIC PROGRAM (VecN/Matrix classes and sigmoid functions removed and rewritten in "Math")
...
public float[] toFloatArray(String[] input, int start) {
  float[] output = new float[input.length-start];
  for(int i = start; i < output.length; i++) {
    output[i-start] = Float.valueOf(input[i]);
  }
  return output;
}
