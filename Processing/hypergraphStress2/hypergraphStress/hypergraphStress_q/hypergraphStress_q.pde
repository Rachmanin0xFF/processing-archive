...
//THIS IS A COPY I DOWNLOADED FROM AN EMAIL TO DAVID
...
boolean calculateValues2 = true; //This will displace each node in the network and find the net impact each node has on the network.
boolean calculateValues3 = false; //This will displace each edge in the network and find the net impact each edge has on the network.
...
void setup() {
  ...
  loadBipartiteData("randomBipartite.csv", 0);
  ...
  String[] s = loadStrings("data/newPermHypergraphs/permHypergraphs4.txt");
...
void loadBipartiteData(String location, int index) {
  ...
  network = new ArrayList<Node>();
  edgenet = new ArrayList<Edge>();
  loadBipartiteData("data/newPermHypergraphs/permHypergraphs" + (index + 1) + ".txt");
  /*
