...
int getBlock(int chunk, int u, int v) {
  ...
  while (u>Chunk.CHUNK_U) {
    chunk++; 
    u-=Chunk.CHUNK_U;
  }
  if(v<0||v>Chunk.CHUNK_V)return 0;//println("There's about to be an error. It's from here, getSprite. awhjfou <- somthing unique to ctrl-f for");
