...
void initMinim() {
  //Load ur sog here
  songInput = loadWAV("D:\\Processing\\Processing 3.x\\ManualFilteringWAV\\musics\\The Dig.wav");
...
void MANGLE() {
  ...
  int lp = 10;
  for(int k = 0; k < 10; k++)
  for(int i = 100; i < songInput.length-100; i++) {
    float sum = 0.f;
    float div = 0.f;
    for(int j = -lp; j < lp; j++) {
      float w = max(0.f, lp-10.f*abs(j)*(0.5*sin((float)i/70000.f)+0.5));
      sum += songInput[j + i]*w;
      div += w;
    }
    sum /= div;
  ...
  for(int i = 100; i < songInput.length-100; i++) {
    songInput[i] = songInput[min(songInput.length - 100, i*2)];
  }
}
