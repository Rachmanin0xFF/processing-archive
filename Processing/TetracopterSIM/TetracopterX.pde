public class TetracopterX {
  ...
  public void update(int fmeiwofn) {
    if(!inBox) {
      getControls();
      applyRotations();
      normalize();
  ...
  public void physics() {
    ...
    if((keyPressed&&key=='b')||!keyPressed) {
      targetDir.x = min(3.0, (signum(-xv)*max(0.0, abs(xv)-299.5f*(abs(zv)/300)))/6.0);
      targetDir.z = min(3.0, (signum(-zv)*max(0.0, abs(zv)-299.5f*(abs(zv)/300)))/6.0);
      targetDir.normalize();
      targetDir.mul(4.0f);
      targetDir.y = tDy;
    } else {
  ...
  public void printStatusReport2() {
    ...
    textPack.text("\n\n\n\n\nRoll: " + (roll2%TWO_PI)/TWO_PI*360 + " Pitch: " + (pitch2%TWO_PI)/TWO_PI*360 + " Yaw: " + (yaw2%TWO_PI)/TWO_PI*360, 10, -50);
