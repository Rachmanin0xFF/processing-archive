...
void createCurtain () {
  ...
  int midHeight = (int) (width/2 - (curtainHeight * restingDistances)/2);
  ...
  for (int y = 0; y <= curtainHeight; y++) { // due to the way particles are attached, we need the y loop on the outside
    for (int x = 0; x <= curtainWidth; x++) { 
      Particle particle = new Particle(new PVector(midHeight + x * restingDistances, y * restingDistances + yStart));
      ...
      if (x != 0)
        particle.attachTo((Particle)(particles.get((x - 1) * (curtainWidth+1) + y)), restingDistances, stiffnesses);
...
/*
      ...
      if ((x != 0) && (y != 0)) 
        particle.attachTo((Particle)(particles.get((y - 1) * (curtainWidth+1) + (x-1))), restingDistances * sqrt(2), 0.1);
      if ((x != curtainWidth) && (y != 0))
        particle.attachTo((Particle)(particles.get((y - 1) * (curtainWidth+1) + (x+1))), restingDistances * sqrt(2), 1);
*/
