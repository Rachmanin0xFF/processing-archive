...
float cameraZoom = 1.4f*winScale; //Camera's zoom.
...
void setup() {
  ...
  size(int(600f*winScale), int(1000f*winScale), P3D);
  ortho();
  smooth(8);
...
void calcNodeColors() {
  for(Node n : network) {
    n.colCount = 0.f;
    n.col = new PVector(0, 0, 0);
  }
  for(Edge e : edgenet) {
    for(int i : e.connectedVerts) {
      network.get(i).col.add(getCommColor(e.commAssignment));
      network.get(i).colCount++;
    }
  }
  for(Node n : network) {
    n.col.mult(1.0f/n.colCount);
  }
}
...
void dispEdgesCommunities() {
  ...
  for(Edge e : edgenet) {
    beginShape(LINES);
    PVector c = getCommColor(e.commAssignment);
    for(int i : e.connectedVerts) {
      PVector q = network.get(i).position;
      stroke(c.x, c.y, c.z, min(100, (fadeBorder-q.y)*fadeSpeed+100));
      vertex(q.x, q.y, q.z);
      stroke(c.x, c.y, c.z, min(100, (fadeBorder-e.position.y)*fadeSpeed));
      vertex(e.position.x, e.position.y, e.position.z);
    }
    endShape(CLOSE);
  }
  for(Edge n : edgenet) {
    float fad = min(255, (fadeBorder - n.position.y)*fadeSpeed + 255);
    
    PVector col = getCommColor(n.commAssignment);
    strokeWeight(4*winScale/1.5f);
    stroke(col.x*0.9f, col.y*0.9f, col.z*0.9f, fad);
    point(n.position.x, n.position.y, n.position.z);
    strokeWeight(4*winScale/1.5f);
    //stroke(255);
    point(n.position.x, n.position.y, n.position.z);
  ...
  for(Node n : network) {
    ...
    strokeWeight(7*winScale/1.5f);
    stroke(n.col.x*0.5f, n.col.y*0.5f, n.col.z*0.5f, fad);
    //stroke(n.col.x, n.col.y, n.col.z, fad);
    point(n.position.x, n.position.y, n.position.z);
    strokeWeight(3*winScale/1.5f);
    stroke(255, fad);
    point(n.position.x, n.position.y, n.position.z);
  }
}

PVector getCommColor(int comm) {
  //if(1==1) return new PVector((comm*500)%255, (comm*1000)%255, (comm*700)%255);
  //if(1==1) return new PVector(comm*10, comm*10, comm*10);
  PVector[] colors = new PVector[]{
  new PVector(103, 81, 42),
  new PVector(162, 168, 120),
  new PVector(173, 218, 185),
  new PVector(100, 143, 152),
  new PVector(29, 46, 64)
  };
  randomSeed((long)(comm*15712.371501) + mouseX/20);
  int r = 15;
  PVector c = colors[(comm + mouseY/20)%5];
  c.add(new PVector(random(-r, r), random(-r, r), random(-r, r)));
  return c;
}
...
public class Node {
  ...
  int degree = -1;
  PVector position = new PVector(0, 0, 0);
  public float colCount = 0.f;
  public PVector col = new PVector(0, 0, 0);
...
void mouseMoved() {
  println("Color scheme seed: (" + mouseX/20 + ", " + mouseY/20 + ")");
}
