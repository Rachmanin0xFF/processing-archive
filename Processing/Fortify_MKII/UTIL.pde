...
public class VecN {
  public ArrayList<Float> v_list = new ArrayList<Float>();
   ...
   */
  public VecN(float ... components) {
    for(float  d : components) {
      v_list.add(d);
    }
    update_swizzles();
  }
  ...
  /**
   * Returns the component in the vector this function is called on at the specified index.
   * @param index
   */
   ...
   * @param components
   */
  public void add_components(float ... components) {
    for(float  d : components) {
      v_list.add(d);
    }
    update_swizzles();
   ...
   */
  public float  dot(VecN v1, VecN v2) {
    if(v1.v_list.size() != v2.v_list.size()) {
      if(!v1.name.equals("")) System.err.println("Warning at vector " + v1.name);
      System.err.println("Dot product warning! Vectors do not have the same number of components! Assuming components of zero for extra components' respective counterparts!");
    }
   ...
   * @return |v|
   */
  public float  magnitude(VecN v) {
    float  sum = 0.;
    for(float  d : v.v_list) {
      sum += d*d;
    }
    return (float)Math.sqrt(sum);
  }
   ...
   */
  public float  distance_to(VecN v) {
    ...
    for(int i = 0; i < Math.max(v_list.size(), v.v_list.size()); i++) {
      float  a = i < v_list.size() ? v_list.get(i) : 0.;
      float  b = i < v.v_list.size() ? v.v_list.get(i) : 0.;
      sum += (b-a)*(b-a);
    }
    return (float)Math.sqrt(sum);
  }
   ...
   * @return |v2 - v1|
   */
  public float  distance(VecN v1, VecN v2) {
    if(v1.v_list.size() != v2.v_list.size()) {
      if(!v1.name.equals("")) System.err.println("Warning at vector " + v1.name);
      System.err.println("Distance warning! Vectors do not have the same number of components! Assuming components of zero for extra components' respective counterparts!");
    }
    float  sum = 0.;
    for(int i = 0; i < Math.max(v1.v_list.size(), v2.v_list.size()); i++) {
      float  a = i < v1.v_list.size() ? v1.v_list.get(i) : 0.;
      float  b = i < v2.v_list.size() ? v2.v_list.get(i) : 0.;
      sum += (b-a)*(b-a);
    }
    return (float)Math.sqrt(sum);
  }
   ...
   * @return v * r
   */
  public VecN mult(VecN v, float  r) {
    VecN out = new VecN();
    for(int i = 0; i < v.v_list.size(); i++) {
...
public class Matrix {
   ...
   * @return m*d
   */
  public Matrix mult(Matrix m, float  d) {
