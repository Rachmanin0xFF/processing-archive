...
//Maps vlue x from range (inStart, inStop) to range (outStart, outStop). (Redone for double precision)
double map(double x, double inStart, double inStop, double outStart, double outStop) {
  return outStart + (outStop - outStart)*((x - inStart)/(inStop - inStart));
}

//Returns the sign function of x.
...
public String[] to_array(ArrayList<String> input) {
  String[] o = new String[input.size()];
  for(int i = 0; i < o.length; i++) {
    o[i] = new String(input.get(i));
  }
  return o;
}
...
public class VecN {
  ...
  /**
  * Finds the Euclidian length or L^2 norm of the vector the function is called on.
  * @return |vector_function_is_called_on|
  */
  public double length() {
    return distance_to(new VecN(0.0, 0.0, 0.0));
  }
...
/**
* Finds the Euclidian length or L^2 norm of a vector.
* @param v
* @return |v|
*/
public double length(VecN v) {
  return distance(v, new VecN(0.0, 0.0, 0.0));
}
...
public class Matrix {
  ...
  public Matrix(PMatrix3D p) {
    this(new VecN(p.m00, p.m01, p.m02, p.m03),
          new VecN(p.m10, p.m11, p.m12, p.m13),
          new VecN(p.m20, p.m21, p.m22, p.m23),
          new VecN(p.m30, p.m31, p.m32, p.m33));
  }
  ...
  public Matrix copy_mat(Matrix m) {
    ...
    for(int i = 0; i < m.m_list.size(); i++) {
      newMat.m_list.set(i, copy_vecN(m.m_list.get(i)));
    }
    return newMat;
  }
  ...
  public void copy_mat(Matrix source, Matrix dest) {
    ...
    for(int i = 0; i < source.m_list.size(); i++) {
      newMat.m_list.set(i, copy_vecN(source.m_list.get(i)));
    }
    dest = newMat;
  }
...
class GoPad {
  ...
  public void set_value(float x, float y) {
    px = x*w/2.f;
    py = y*h/2.f;
  }
  public void update() {
...
class Button {
  ...
  boolean was_mouse_in_bounds = false;
  boolean too_da_right = false;
  boolean use_image = false;
  boolean override_input = false;
  PImage img;
  ...
  boolean inMyBounds() {
    return is_in_bounds_inclusive(mouseX, mouseY, x, y, w, h);
  }
  public void update() {
    if(mousePressed) {
  ...
  public void display() {
    ...
    if(too_da_right) {
      fill(border_color);
      if(is_on) rect(x + 2, y + 2, w - 4, h - 4);
      textAlign(LEFT, CENTER);
      text(name, x + w + 10, y + h/2);
    } else {
      textAlign(CENTER, CENTER);
      text(name, x + w/2, y + h/2);
    }
    if(use_image) {
      if(changed) img.filter(INVERT);
      image(img, x + 2, y + 2, w - 4, h - 4);
    }
    textAlign(LEFT, TOP);
  }
  public void set_theme(Theme t) {
...
public class Slider {
  ...
  //Input is me.getCount();
  void update_scroll(int count) {
    if(is_in_bounds_inclusive(mouseX, mouseY, x, y, w, h)) {
      float mapped = -((float)count)/10.f;
      value2 += mapped*0.05f;
      value2 = max(0.0, min(1.0, value2));
      value = map(value2, 0.f, 1.f, minimum, maximum);
    }
