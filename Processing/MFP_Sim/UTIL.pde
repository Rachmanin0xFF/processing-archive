...
boolean OVERRIDE_VECTOR_ERRORS = false;
...
public class VecN {
  ...
  public void add(VecN v) {
    if(!OVERRIDE_VECTOR_ERRORS && v.v_list.size() != v_list.size()) {
  ...
  public double distance_to(VecN v) {
    if(!OVERRIDE_VECTOR_ERRORS && v_list.size() != v.v_list.size()) {
      if(!name.equals("")) System.err.println("Warning at vector " + name);
...
public VecN add(VecN v1, VecN v2) {
  VecN v = new VecN();
  if(!OVERRIDE_VECTOR_ERRORS && v1.v_list.size() != v2.v_list.size()) {
...
public VecN cross_vectors(VecN... vectors) {
  ...
  for(VecN v : vectors) {
    if(!OVERRIDE_VECTOR_ERRORS && vector_components != v.v_list.size()) {
  ...
  if(!OVERRIDE_VECTOR_ERRORS && vectors.length != vector_components - 1 && vectors.length != vector_components) {
    if(!vectors[0].name.equals("")) System.err.println("Error at vector " + vectors[0].name);
