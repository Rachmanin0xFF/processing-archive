...
//THIS CODE IS OLD AND MANGLED AND MESSY BE WARNED!!!!
//DO NOT LOOK FOR THIS CODE FOR GUIDANCE AS IT WILL ONLY GIVE YOU PAIN AND SUFFERING!!!! (this may be an exaggeration)
//I should really rewrite this whole thing.

//It used to be 3D, made some quick edits to downgrade it to 2D (just for fun?????), things are pretty crazy in the source.

//Okay so basically- A Quanta is a particle/mass/body. "quantas" is an arraylist full of Quanta. All bodies in 
//quantas are attracted to one another. A "Node" is an 'abstract' item, designed to be a node in the k-D tree (though in
//this code, the k-D tree is pretty much a 2-D tree. Nodes have children (other nodes lower down in the tree, sub-nodes 
//of that node), and a parent (the super-node of that Node). Each node has a unique index in the arraylist of Nodes called nodes.
//Nodes can be linked to one another if they know each other's index in the list. Nodes can also be linked to 
//multiple or single Quanta. Each Quanta also has a unique index in quantas, if a node knows this index they
//can get information about the quanta.

//stuff starts to get commented a little after draw().

/*Update 1.1 - Graphics ENHANCED, everything will look at least 20% cooler now. Fixed blending, made camera zoom dependent on the standard deviation of the set of particles, added a createQuantainerSperoidica() function.*/
...
boolean camRoll = false; //Set true to have an automated camera when rendering.
...
float treeOpacity = 60f;
float lineLength = 5f;
float particleAlpha = 250f;
...
boolean spawnInCircle = true;
float circleSize = 200.f;
...
float eventHorizon = 10f; //Accurateness constant two. This is the "event horizon" of accurateness. If the distance between two particles is less than this number, those two particles will be normally attracted to one another.
float gravityConstant = 128f; //This is the gravity constant. Smaller = less gravity, bigger = more gravity.
int particleListSize = 1000; //This is the number of particles in the simulation.
int frameDivide = 4; //THIS IS THE NUMBER OF SUB-FRAMES EVERYTHING IS SPLIT INTO. Set to 1 for highest accuracy.
...
//This interprets NODE NAVIGATION COMMANDS, I did this so I wouldn't have to write out stuff like "nodes.get(nodes.get(parent).parent).get(children(0))". Bleh.
...
//All the createQuantainer() functions fill the list quantas with nodes. Most of them are 3D creation functions, and won't make much sense in 2D.
//For example, createQuantainerSpheroidica() starts the particles out in a sphere, while createQuantainer() will start them out in a cube.
void createQuantainer() {
  ...
  for(int i = 0; i < particleListSize; i++) {
    ...
    PVector vel = new PVector(x, y, z*0.f);
    vel.normalize();
    vel.mult(random(10f));
    PVector pos = new PVector(x, y, z*0.f);
    pos.normalize();
    ...
    if(spawnInCircle) {
      ...
      float r = sqrt(random(1.f))*circleSize;
      x = r*cos(theta);
      y = r*sin(theta);
      z = 0.f;
    }
...
//Okaaaaayyyyy.... k-D tree stuff stuff.
//These are recursive functions that repeatedly split a group of particles in half again and again.
//Basically, they look at all the particles, split them into 2 new groups evenly across the Y-axis, then split each of those groups
//into 2 new groups evenly across the X-axis, then all of those groups split again, and this process repeats until the particle count per group gets to a certain number, or
//a maximum splitting level is reached. It does all this by making 2 new nodes for each split, and making those nodes the children of the original group node. Once the groups get
//small enough, the nodes start to get linked to actual particles.

//Actually, a quadtree/octree probably would be quicker here. This is kind of a silly way to write a k-D tree. (multiple split functions and whatnot)
//It's more readable than the alternative, though.
...
void treeSplitRecurX(ArrayList<Quanta> q, int nodeID, int x0, int y0, int z0, int x1, int y1, int z1, int iter, float wid) {
  ...
  //Only do the recursive splitty-thing if there are more than <bucketSize> particles in the input group, and we are not over out maximum number of iterations.
  if(iter<maxIterations&&q.size()>bucketSize) {
    if(q.size() > 1) {
      //Calculate the average x-component of the input group/list.
      float x = 0;
      for(Quanta k : q)
        x += k.x;
      x /= q.size();
      ArrayList<Quanta> A1 = new ArrayList<Quanta>(); //The left side
      ArrayList<Quanta> A2 = new ArrayList<Quanta>(); //The right side
      for(int i = 0; i < q.size(); i++) {
        if(q.get(i).x <= x)
          A1.add(q.get(i)); //If the particle's x-component is less than the average x-component, put it in the left side list.
        else
          A2.add(q.get(i)); //If it's not, put it in the right side list.
      }
      //Add children to the parent node for each of the splits we are about to do.
      int id = nodes.get(nodeID).addNode();
      int id2 = nodes.get(nodeID).addNode();
      //Feed these ids of these newly-born node-fetuses into the next big split.
      treeSplitRecurY(A1, id,  x0, y0, z0, (int)x, y1, z1, iter+1, wid/1.259); // <-- RECURSIVE!!! (split the left and right side groups in half vertically).
      treeSplitRecurY(A2, id2, (int)x, y0, z0, x1, y1, z1, iter+1, wid/1.259); // <-- RECURSIVE!!!
    } else {
      //If there is one quanta in our input group/list...
      if(q.size()>0) {
        //Link it to the node of that list and wrap things up.
        q.get(0).setNode(nodeID);
        nodes.get(nodeID).addLink(q.get(0).id);
      }
    }
  } else {
    //If we're done splitting...
    for(Quanta p : q) {
      //Link all the quanta in the input group/list to their group/list's node and wrap things up.
      p.setNode(nodeID);
      nodes.get(nodeID).addLink(p.id);
    }
  }
  //Draw cool tech-y stuff.
  if(showTree) doLines(x0, y0, z0, x1, y1, z1);
}


//Pretty much the same as the X, but with the Y. Woo hoo.
void treeSplitRecurY(ArrayList<Quanta> q, int nodeID, int x0, int y0, int z0, int x1, int y1, int z1, int iter, float wid) {
  ...
  if(iter<maxIterations&&q.size()>bucketSize) {
    if(q.size() > 1) {
      ...
      int id = nodes.get(nodeID).addNode();
      int id2 = nodes.get(nodeID).addNode();
      treeSplitRecurX(A1, id, x0, y0, z0, x1, (int)y, z1, iter+1, wid/1.259);
      treeSplitRecurX(A2, id2, x0, (int)y, z0, x1, y1, z1, iter+1, wid/1.259);
    } else {
      if(q.size()>0) {
        q.get(0).setNode(nodeID);
        nodes.get(nodeID).addLink(q.get(0).id);
...
//Stadard deviation functions (TODO: make more universal function that returns PVector)
//These are used when adaptive zooming is enabled.
...
//Draw a box.
//So exciting.
//2014 Best function award nominee.
...
class Node {
  ...
  //Centerpoint of the nodes it contains (including those it's children contain).
  float xc;
  float yc;
  float zc;
  float mass; //Pretty much the number of nodes in it, I think?
  float regionWidth; //Duh.
  //Yeahhhhh.... I like to override constructors.
  ...
  //This is a recursive function that you run on the top node in the tree, it will propegate downward until it reaches the bottom nodes (the ones linked to
  //particles), and tell them to do the attraction thing. I think.
  public void attract2(int qLink, int iter) {
    ...
    if((!(children.size()==0))&&(mass/f>accuratenessConstant||f<eventHorizon)) {
      for(int i : children)
        nodes.get(i).attract2(qLink, iter+1); // <-- RECURSIVE!!
    } else {
  ...
  //Short for calculate center. Recursive and stuffs.
  public vec4 calcC() {
    if(children.size()==0) {
      mass = 1f;
      ...
      float f = 0;
      for(int q : children) {
        vec4 v = nodes.get(q).calcC(); // <-- RECURSIVE!!
        f += v.w;
        a.add(v, v.w);
      }
      float tx = a.x/f;
  ...
  //Came in 5th place for best function awards 2013.
  public void printID() {
    println("ID: " + id);
  ...
  //Apparently this name was already a thing.
...
//The quanta class. Surprisingly (or not so) boring.
public class Quanta {
  float x;
  float y;
  float z;
  ...
  int id = -1;
  int node = -1; //The Node this Quanta is linked to.
  float p = 0;
...
//hoo hoo hoo! 4-component vectors! hoo hoo hoo!
public class vec4 {
  float x;
  float y;
  float z;
