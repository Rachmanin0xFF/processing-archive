    ...
    if(rw <= w || rh <= h) {
      for(int i = tx; i < tw; i++) {
        for(int j = ty; j < th; j++) {
          ...
          PVector p = get(xc, yc);
          stroke(p.x, p.y, p.z, 255);
          point(i, j);
        }
      }
    } else {
      strokeCap(SQUARE);
      strokeWeight(ceil(rw/w));
      for(int i = 0; i < w; i++) {
        for(int j = 0; j < h; j++) {
...
public void display_data_2D_simple(int[][] data, int w, int h, float x, float y, float rw, float rh) {
  int tw = int(rw);
  int th = int(rh);
  int tx = int(x);
  int ty = int(y);
  ...
  for(int i = 0; i < w; i++) {
    for(int j = 0; j < h; j++) {
      float xc = map(float(i), 0, w, float(tx), float(tw));
      float yc = map(float(j), 0, h, float(ty), float(th));
      color p = color(data[i][j], data[i][j], data[i][j], 255);
      stroke(p);
      point(xc, yc);
    }
  }
...
//Arguments are as follows:               datawidth dataheight  drawx  drawy   drawwidth drawheight
public void display_data_2D_simple(float[][] data, int w, int h, float x, float y, float rw, float rh) {
  int tw = int(rw);
  int th = int(rh);
  int tx = int(x);
  int ty = int(y);
  ...
  for(int i = 0; i < w; i++) {
    for(int j = 0; j < h; j++) {
      float xc = map(float(i), 0, w, float(tx), float(tw));
      float yc = map(float(j), 0, h, float(ty), float(th));
      color p = color(data[i][j] + 255/2.f, 255);
      stroke(p);
      point(xc, yc);
    }
  }
...
public class Slider {
  ...
  void update() {
    ...
    if(!(mousePressed && only_left_mouse_button && mouseButton != LEFT)) {
      ...
      boolean b = false;
      if(horizon)
        if(mousePressed && is_in_bounds_inclusive(mouseX, mouseY, map(value, 0.0f, 1.0f, x, x + w - drag_bar_length), y, drag_bar_length, h))
          dragging = true;
      else
        if(mousePressed && is_in_bounds_inclusive(mouseX, mouseY, x, map(value, 0.0f, 1.0f, y, y + h - drag_bar_length), w, drag_bar_length))
            dragging = true;
      if(!mousePressed)
        dragging = false;
    }
    ...
    if(dragging) {
      if(horizon)
        value += deltamouse_x/(w - drag_bar_length);
    }
  ...
  void display() {
    ...
    if(dragging)
      fill(border_color);
    if(horizon) {
      rect(map(value, 0.0f, 1.0f, x, x + w - drag_bar_length), y, drag_bar_length, h, radius);
      fill(border_color);
      ...
      if(display_value) {
        ...
        if(round_digits) {
          String s = round_to(map(value, 0.f, 1.f, minimum, maximum), digits_to_round_to);
          text(s, map(value, 0.0f, 1.0f, x + drag_bar_length/2, x + w - drag_bar_length/2), y + h/2 + 2);
        } else {
          text(map(value, 0.f, 1.f, minimum, maximum), map(value, 0.0f, 1.0f, x + drag_bar_length/2, x + w - drag_bar_length/2), y + h/2 + 2);
        }
      }
    } else {
      rect(x, map(value, 0.0f, 1.0f, y, y + h - drag_bar_length), w, drag_bar_length, radius);
    }
    textAlign(LEFT, TOP);
  }
}
